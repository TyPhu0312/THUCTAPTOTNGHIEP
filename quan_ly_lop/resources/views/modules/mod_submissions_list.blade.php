...
$target_id = $exam_id ?? $assignment_id ?? null;

// If no submissions data passed, use empty collection
...
<div class="p-4 bg-white rounded-lg shadow-md">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h2 class="text-xl font-bold text-gray-800">Danh sách bài nộp <span class="text-blue-600">({{ $type }})</span></h2>
                ...
                <tr class="hover:bg-gray-50 submission-row transition-colors"
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <a href="{{ asset('storage/' . $submission->file_path) }}" target="_blank" 
                           class="flex items-center text-blue-600 hover:text-blue-800 hover:underline">
                            ...
                            {{ Str::limit(basename($submission->file_path), 20) }}
                        </a>
                        @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            <a href="{{ route('submissions.show', $submission->submission_id ?? $submission->id) }}" 
                            ...
                            <a href="{{ route('submissions.edit', $submission->submission_id ?? $submission->id) }}" 
    ...
    @if(isset($submissions) && method_exists($submissions, 'links') && $submissions->hasPages())
    <div class="mt-4">
        {{ $submissions->links() }}
    </div>
    @endif
